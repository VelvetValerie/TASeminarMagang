...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start mb-6">
        ...
        <div class="border-2 border-black bg-white shadow-sm flex flex-col min-h-[110px]">
            <h3 class="text-xs sm:text-sm font-bold text-gray-900 px-3 py-1.5 border-b-2 border-black bg-gray-50">
                Kegiatan yang Terdaftar di Database
            </h3>
            <div class="p-3 text-xs sm:text-sm space-y-1 font-medium text-gray-800">
                @forelse($kegiatan->take(4) as $idx => $k)
                    <p>{{ $idx + 1 }}. {{ $k->nama_keg }} ({{ $k->jenis->nama_jeniskeg ?? '-' }}) - {{ \Carbon\Carbon::parse($k->tanggal_mulai)->format('d/m/Y') }}</p>
                @empty
                    <p class="text-gray-500">Tidak ada kegiatan berlangsung.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 pb-4 border-b-2 border-black relative">
            ...
            <div id="filterCalendarModal" class="hidden absolute right-0 top-full mt-2 w-80 bg-white border-2 border-black p-4 shadow-2xl z-30">
                ...
                <div class="mt-4 border-2 border-black p-3 space-y-3 bg-white">
                    ...
                    <label class="flex items-center space-x-3 text-xs sm:text-sm font-semibold cursor-pointer">
                        <input type="checkbox" data-filter="sekolah-kedinasan" checked class="cal-filter-checkbox w-4 h-4 accent-black rounded border-2 border-black">
                        <span>Sekolah Kedinasan</span>
                    </label>
                    <label class="flex items-center space-x-3 text-xs sm:text-sm font-semibold cursor-pointer">
                ...
                <tbody class="text-sm font-semibold">
                    <tr class="border-b-2 border-black h-28">
                        ...
                        <td class="border-r-2 border-black p-2 align-top relative">
                            <span>3</span>
                            <div class="cal-event absolute top-8 left-3 w-[190%] h-6 bg-gray-300 border-2 border-black rounded-full flex items-center justify-center z-20 shadow-xs pointer-events-none" data-category="tes-casn">
                                <span class="text-xs font-bold text-gray-900 tracking-wide">Tes CASN</span>
                            </div>
                        </td>
                        <td class="border-r-2 border-black p-2 align-top relative">
                            <span>4</span>
                            <div class="cal-event absolute top-16 left-3 w-[190%] h-6 bg-gray-300 border-2 border-black rounded-full flex items-center justify-center z-10 shadow-xs pointer-events-none" data-category="sekolah-kedinasan">
                                <span class="text-xs font-bold text-gray-900 tracking-wide">Tes CAT</span>
                            </div>
                        </td>
                        <td class="border-r-2 border-black p-2 align-top relative">
                            <span>5</span>
                            <div class="cal-event mt-1 mx-auto w-[85%] h-6 bg-gray-600 border-2 border-black text-white text-xs font-bold rounded-full flex items-center justify-center shadow-xs" data-category="tes-casn">
                                Tes CASN
                            </div>
                        </td>
                    ...
                    <tr class="border-b-2 border-black h-28">
                        ...
                        <td class="border-r-2 border-black p-2 align-top relative">
                            <span>10</span>
                            <div class="cal-event mt-4 mx-auto w-[85%] h-6 bg-gray-600 border-2 border-black text-white text-xs font-bold rounded-full flex items-center justify-center shadow-xs" data-category="tes-casn">
                                Tes CASN
                            </div>
                        </td>
    ...
    <script>
        ...
        calBtn.addEventListener('click', (e) => { e.stopPropagation(); calModal.classList.toggle('hidden'); });
