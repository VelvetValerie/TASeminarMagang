...
<script>
    (function() {
        ...
        function renderPagination() {
            ...
            let html = '';
            html += `<button onclick="changePageComp(${currentPage - 1})" class="border-2 border-black px-3 py-1 bg-white font-bold text-xs uppercase hover:bg-gray-100 ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}" ${currentPage === 1 ? 'disabled' : ''}>Prev</button>`;

            for (let i = 1; i <= totalPages; i++) {
                const isActive = i === currentPage;
                html += `<button onclick="changePageComp(${i})" class="border-2 border-black w-8 py-1 ${isActive ? 'bg-black text-white' : 'bg-white text-gray-900 hover:bg-gray-100'} font-bold text-xs cursor-pointer">${i}</button>`;
            }

            html += `<button onclick="changePageComp(${currentPage + 1})" class="border-2 border-black px-3 py-1 bg-white font-bold text-xs uppercase hover:bg-gray-100 ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}" ${currentPage === totalPages ? 'disabled' : ''}>Next</button>`;

            paginationContainer.innerHTML = html;
        }

        window.changePageComp = function(newPage) {
            const totalPages = Math.ceil(allData.length / itemsPerPage);
